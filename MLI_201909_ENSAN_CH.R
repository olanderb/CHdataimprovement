...
data <- data  %>% select(adm1_name = q11a_nom_region, #first adminstrative division
                                            adm2_name = q12_nom_cercle, #second administrative division
                                            HDDS = SDAM_ENSAN022018, #Household Dietary Diversity Score
                                            FCG = FCClass, #Food Consumption Groups from the Food Consumption Score
                                            HHS = HHSscore, #Household Hunger Score
                                            rCSI = CSI_reduit, #reduced coping strategies
                                            LHCS = max_coping_strat, #livelihood coping strategies
...
LHCS = case_when(
  LHCS == "Pas de stratégies" ~ "NoStrategies", # Put accents to stratigies so that the code works properly
  LHCS == "Stratégies de stress" ~ "StressStrategies",
  LHCS == "Stratégies de crise" ~ "CrisisStrategies",
  LHCS == "Stratégies d'urgence" ~ "EmergencyStrategies"
),
CH_HDDS = case_when(
  HDDS >= 5 ~ "Phase1", 
  HDDS == 4 ~ "Phase2",       
...
CH_HDDStable <- HDDStable %>% mutate(indicator = "HDDS", 
                                     ...
                                     finalphase = case_when(
                                       ...
                                       select(indicator, adm1_name, adm2_name, HDDS_Phase1 = Phase1, HDDS_Phase2 = Phase2, HDDS_Phase3 = Phase3, HDDS_Phase4 = Phase4, HDDS_Phase5 = Phase5, HDDS_finalphase = finalphase) #select only relevant variables, rename them with indicator name and order in proper sequence
...
CH_FCGtable <-  FCGtable %>%  mutate(indicator = "FCG", PoorBorderline = Poor + Borderline, finalphase = case_when(
  ...
  select(indicator, adm1_name, adm2_name, FCG_Poor = Poor, FCG_Borderline = Borderline, FCG_Acceptable = Acceptable, FCG_finalphase = finalphase) #select only relevant variables and order in proper sequence
...
CH_HHStable <- HHStable %>% mutate(indicator = "HHS", phase2345 = `Phase2` + `Phase3` + `Phase4` + `Phase5`,
                                   ...
                                   finalphase = case_when(
                                     ...
                                     TRUE ~ 1)) %>% 
                                    select(indicator, adm1_name, adm2_name, HHS_Phase1 = Phase1, HHS_Phase2 = Phase2, HHS_Phase3 = Phase3, HHS_Phase4 = Phase4, HHS_Phase5 = Phase5, HHS_finalphase = finalphase)
...
CH_rCSItable <- rCSItable %>% mutate(indicator = "rCSI", 
                                     ...
                                     finalphase =
                                       case_when(
                                         ...
                                         TRUE ~ 1
                                       )) %>% select(indicator, adm1_name, adm2_name, rCSI_Phase1 = Phase1, rCSI_Phase2 = Phase2, rCSI_Phase3 =Phase3, rCSI_finalphase = finalphase)


#Livelihood Coping Strategies
LHCStable <- data %>% 
  group_by(adm1_name, adm2_name) %>%
  count(LHCS,wt=Weight) %>%
  drop_na() %>%
  mutate(n = 100 * n / sum(n)) %>%
  ungroup() %>%
  spread(key = LHCS, value = n) %>% replace(., is.na(.), 0) %>% mutate_if(is.numeric, round, 1)
#Apply the Cadre Harmonise rules for phasing the Livelihood Coping Strategies 
CH_LHCStable <- LHCStable %>% mutate(indicator = "LHCS", stresscrisisemergency = StressStrategies + CrisisStrategies + EmergencyStrategies,
                                     ...
                                     finalphase = case_when(
                                       ...
                                       NoStrategies >= 80 ~ 1
                                     )) %>% select(indicator, adm1_name, adm2_name, LHCS_NoStrategies = NoStrategies, LHCS_StressStrategies = StressStrategies, LHCS_CrisisStategies = CrisisStrategies, LHCS_EmergencyStrategies = EmergencyStrategies, LHCS_finalphase = finalphase)
...
choc_subitable <- data %>% 
  group_by(adm1_name, adm2_name) %>%  mutate(indicator = "Pendant les six dernier mois, le menage a-t-il subi un choc?") %>%
  count(choc_subi) %>%
  drop_na() %>%
  mutate(n = 100 * n / sum(n)) %>%
  ungroup() %>%
  spread(key = choc_subi, value = n) %>% replace(., is.na(.), 0) %>% mutate_if(is.numeric, round, 1) %>%
  select(adm1_name, adm2_name, `01_choc_subi_Non` = Non, `01_choc_subi_Oui` = Oui)
...
matrice_intermediaire <- bind_cols(
  ...
  select(CH_HDDStable,-c("indicator","adm1_name","adm2_name")),# select all variables except indicator, adm1_name and adm2_name because the latter two are already selected from the table CH_FCGtable
  select(CH_HHStable,-c("indicator","adm1_name","adm2_name")),
  select(CH_LHCStable,-c("indicator","adm1_name","adm2_name")),
  select(CH_rCSItable,-c("indicator","adm1_name","adm2_name")),
  select(choc_subitable,-c("adm1_name","adm2_name"))
)
...
matrice_intermediaire <- matrice_intermediaire %>% 
  select(adm1_name,adm2_name,Population,Geocode,everything())
